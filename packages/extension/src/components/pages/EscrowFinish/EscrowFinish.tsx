...
import { EscrowFinish as EscrowFinishTransaction } from 'xrpl';
...
interface Params {
  id: number;
  transaction: EscrowFinishTransaction | null;
}

export const EscrowFinish: FC = () => {
  ...
  const { fetchedData } = useFetchFromSessionStorage(
    ...
    fetchedData: EscrowFinishRequest | undefined;
  };

  const sendMessageToBackground = useCallback(
    (message: ReceiveEscrowFinishBackgroundMessage) => {
  ...
  useEffect(() => {
    ...
    const fulfillment = 'fulfillment' in fetchedData ? fetchedData.fulfillment : undefined;
  ...
  const handleConfirm = useCallback(() => {
    ...
    escrowFinish(params.transaction as EscrowFinishTransaction)
      .then((response) => {
  ...
  }, [params, escrowFinish, sendMessageToBackground, createMessage]);
  ...
  return (
    <TransactionPage
      title="Finish Escrow"
      description="Please review the escrow finish transaction below."
