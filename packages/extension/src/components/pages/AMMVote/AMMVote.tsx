...
import { AMMVote as AMMVoteTx, Currency } from 'xrpl';
...
interface Params {
  id: number;
  transaction: AMMVoteTx | null;
}

export const AMMVote: FC = () => {
  ...
  const { fetchedData } = useFetchFromSessionStorage(
    ...
    fetchedData: AMMVoteRequest | undefined;
  };

  const sendMessageToBackground = useCallback(
    (message: ReceiveAMMVoteBackgroundMessage) => {
  ...
  useEffect(() => {
    ...
    if (fetchedData.Asset && fetchedData.Asset2 && fetchedData.TradingFee !== undefined) {
    ...
    const transaction = buildAMMVote(
      {
  ...
  const handleConfirm = useCallback(() => {
    ...
    ammVote(params.transaction as AMMVoteTx)
      .then((response) => {
  ...
  }, [params, ammVote, sendMessageToBackground, createMessage]);
