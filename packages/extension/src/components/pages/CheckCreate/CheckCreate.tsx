...
import { CheckCreate as CheckCreateTransaction } from 'xrpl';
...
interface Params {
  id: number;
  transaction: CheckCreateTransaction | null;
}

export const CheckCreate: FC = () => {
  ...
  const { fetchedData } = useFetchFromSessionStorage(
    ...
    fetchedData: CheckCreateRequest | undefined;
  };

  const sendMessageToBackground = useCallback(
    (message: ReceiveCheckCreateBackgroundMessage) => {
  ...
  useEffect(() => {
    ...
    const sendMax =
      'sendMax' in fetchedData ? parseAmount(fetchedData.sendMax, null, null, '') : undefined;
  ...
  const handleConfirm = useCallback(() => {
    ...
    checkCreate(params.transaction as CheckCreateTransaction)
      .then((response) => {
  ...
  }, [params, checkCreate, sendMessageToBackground, createMessage]);
  ...
  return (
    <TransactionPage
      title="Create Check"
      description="Please review the check transaction below."
