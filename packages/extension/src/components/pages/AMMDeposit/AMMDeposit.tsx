...
import { AMMDeposit as AMMDepositTx, Currency } from 'xrpl';
...
interface Params {
  id: number;
  transaction: AMMDepositTx | null;
}

export const AMMDeposit: FC = () => {
  ...
  const { fetchedData } = useFetchFromSessionStorage(
    ...
    fetchedData: AMMDepositRequest | undefined;
  };

  const sendMessageToBackground = useCallback(
    (message: ReceiveAMMDepositBackgroundMessage) => {
  ...
  useEffect(() => {
    ...
    const transaction = buildAMMDeposit(
      {
        ...
        ...(fetchedData.LPTokenOut && { LPTokenOut: fetchedData.LPTokenOut }),
  ...
  const handleConfirm = useCallback(() => {
    ...
    ammDeposit(params.transaction as AMMDepositTx)
      .then((response) => {
  ...
  }, [params, ammDeposit, sendMessageToBackground, createMessage]);
