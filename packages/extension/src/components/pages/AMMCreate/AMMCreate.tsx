...
import { AMMCreate as AMMCreateTx } from 'xrpl';
...
interface Params {
  id: number;
  transaction: AMMCreateTx | null;
}

export const AMMCreate: FC = () => {
  ...
  const { fetchedData } = useFetchFromSessionStorage(
    ...
    fetchedData: AMMCreateRequest | undefined;
  };

  const sendMessageToBackground = useCallback(
    (message: ReceiveAMMCreateBackgroundMessage) => {
  ...
  useEffect(() => {
    ...
    if (!fetchedData.Amount || !fetchedData.Amount2 || fetchedData.TradingFee === undefined) {
      setIsParamsMissing(true);
      return;
    }

    const transaction = buildAMMCreate(
      {
        ...parseBaseParamsFromStoredData(fetchedData),
        Amount: fetchedData.Amount,
        Amount2: fetchedData.Amount2,
        TradingFee: fetchedData.TradingFee
      },
      wallet
    );
  ...
  const handleConfirm = useCallback(() => {
    ...
    ammCreate(params.transaction as AMMCreateTx)
      .then((response) => {
  ...
  }, [params, ammCreate, sendMessageToBackground, createMessage]);
  ...
  return (
    <TransactionPage
      title="Create AMM Pool"
      description="Create a new Automated Market Maker pool with two assets."
      approveButtonText="Create Pool"
